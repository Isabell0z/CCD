...
# load data
    ...
    new_user_train, new_user_valid, new_user_test = get_train_valid_test_mat_for_new_users(before_user, current_user,current_train_mat,current_valid_mat,current_test_mat)
    ...
    # load student model
    Distill_Student_rootpath = f"../ckpt/{args.dataset}/Student/{args.model}/Method/Distilled"
    Distill_Student_path = os.path.join(Distill_Student_rootpath, f"TASK_{before_task}.pth")
    Distill_model_weight, Distill_score_mat, Distill_sorted_mat = load_saved_model(Distill_Student_path, gpu)
    ...
    Distill_Student_model = get_model(before_user,before_item,before_SNM,gpu,args,model_type,Distill_model_weight).to(gpu)
    ...
    if before_task == 0:
        ...
        P_proxy_rootpath = f"../ckpt/{args.dataset}/Student/{args.model}/Method/Plasticity"
        S_proxy_rootpath = f"../ckpt/{args.dataset}/Student/{args.model}/Method/Stability"
        ...
        P_proxy = get_model(proxy_get_user,proxy_get_item,proxy_SNM,gpu,args,model_type,P_proxy_weight)
        S_proxy = get_model(proxy_get_user,proxy_get_item,proxy_SNM,gpu,args,model_type,S_proxy_weight)
        del proxy_R,proxy_SNM

        # update proxy using current distillation data
        S_proxy = merge_model(S_proxy, Distill_Student_model, wme=True, b_weight=args.s_weight, p_weight=1 - args.s_weight).to(gpu)
        P_proxy = merge_model(P_proxy, Distill_Student_model, wme=True, b_weight=args.p_weight, p_weight=1 - args.p_weight).to(gpu)
...
# step1 : Learning with new interactions
    ...
    if before_task == 0 :
        Teacher_path = f"../ckpt/{args.dataset}/Teacher/base_teacher/Ensemble/TASK_0_score_mat.pth"
    else:
        Teacher_path = f"../ckpt/{args.dataset}/Teacher/using_student_{args.model}/Method/Ensemble/TASK_{before_task}_score_mat.pth"
    Teacher_score_mat = torch.load(Teacher_path,map_location=gpu)["score_mat"].detach().cpu()
    ...
    BPR_train_data = implicit_CF_dataset(before_user,before_item,before_train_mat,args.nns,current_interaction,exclude_top_items)
    train_data = DataLoader(BPR_train_data,batch_size=args.bs,shuffle = True, drop_last = False)
    ...
    S_model.update(before_user_ids, before_item_ids,
                 before_user_mapping, before_item_mapping,
                 before_rating_mat, new_user, new_item,
...
# step3 : train model (replay learning)  and the overall objective for student update
    ...
    for epoch in range(args.max_epoch):
        ...
        for batch in train_data:
            ...
            # updata model
            batch_loss = S_model.base_model.get_loss(embedding)
            optimizer.zero_grad()
            batch_loss.backward()
            optimizer.step()
        ...
        CF_time = time.time()
        print(f"epoch_CF_loss = {round(CF_loss / len(train_data), 4)}, CF_time = {CF_time - start_time:.4f} seconds",end=" ")
        ...
        if args.replay_learning:
            ...
            # shuffle (may delete???)
            shuffled_list = random.sample(replay_learning_data, len(replay_learning_data))
            ...
            for id in range(iteration):
                ...
                user_embedding, item_embedding = S_model.base_model.get_embedding()
            ...
            print(f"epoch_replay_learning_loss = {round(replay_learning_loss / iteration, 4)}, replay_learning_lambda = {replay_learning_lambda:.5f}, replay_learning_time = {replay_learning_time - CF_time:.4f} seconds",end=" ")
