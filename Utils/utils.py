...
def get_VAE_replay_learning_loader_integrate_with_R(
    replay_learning_dataset, R, total_user, total_item, args
):
    ...
    VAE_train_dataset = implicit_CF_dataset_AE(
        total_user, total_item, rating_mat=None, is_user_side=True, R=Integrate_R
    )
    VAE_train_loader = DataLoader(
        VAE_train_dataset, batch_size=args.bs, shuffle=True, drop_last=False
    )
...
def get_info_for_VAE_Teacher(
    before_user_ids, before_item_ids, present_user_ids, present_item_ids, before_R
):
    """Get the basic information of Teacher(VAE)"""
    common_user_ids = get_common_ids(
        before_user_ids, present_user_ids
    )  # 순서 바뀌어도 상관 없음.
    ...
    u_ids, i_ids = torch.where(torch.eq(common_R, 1.0))
...
def get_info_for_BPR_Teacher(before_user_ids, before_item_ids, before_train_dict):
    ...
    before_rating_mat = make_before_rating_mat(
        before_train_dict,
        before_user_mapping,
        before_item_mapping,
        len(before_user_ids),
        len(before_item_ids),
    )  # applying before user/item mapping for squeezing
...
def get_RRD_IR_BPR_dataset(
    ...
    # RRD dataset
    RRD_train_dataset = RRD_dataset_simple(
        T_RRD_interesting_items, T_score_mat_for_RRD, num_uninteresting_items=nui
    )
    IR_reg_train_dataset = IR_RRD_dataset_simple(
        T_RRD_interesting_users, T_score_mat_for_RRD.t(), num_uninteresting_users=nuu
    )

    negatvie_exclude = torch.cat([negatvie_exclude, T_RRD_interesting_items], dim=1)

    # CF dataset
    BPR_train_dataset = implicit_CF_dataset(
        p_total_user,
        p_total_item,
        p_train_mat,
        nns,
...
def get_negative_exclude(
    ...
    if distillation_idx > 0:
        ...
        # S_proxy
        S_proxy_task_path = os.path.join(
            load_S_proxy_dir_path, f"TASK_{distillation_idx-1}.pth"
        )
        S_score_mat = (
            torch.load(S_proxy_task_path, map_location=gpu)["score_mat"].detach().cpu()
        )
        S_interesting_items = get_interesting_items_after_filtering(
            S_score_mat, p_train_dict
        )
        ...
        if distillation_idx >= 2:
            P_proxy_task_path = os.path.join(
                load_P_proxy_dir_path, f"TASK_{distillation_idx-1}.pth"
            )
            P_score_mat = (
                torch.load(P_proxy_task_path, map_location=gpu)["score_mat"]
                .detach()
                .cpu()
            )
            P_interesting_items = get_interesting_items_after_filtering(
                P_score_mat, p_train_dict
            )
            P_interesting_items = expand_mat(
                P_interesting_items, (p_total_user, 40), -1
            )
        ...
        CL_model_task_path = os.path.join(
            load_CL_model_dir_path, f"TASK_{distillation_idx}.pth"
        )
        CL_score_mat = (
            torch.load(CL_model_task_path, map_location=gpu)["score_mat"].detach().cpu()
        )
        CL_interesting_items = get_interesting_items_after_filtering(
            CL_score_mat, p_train_dict
        )
...
def get_train_valid_test_mat_for_new_users(
...
def get_train_valid_test_mat(
    ...
    train_mat = make_rating_mat(
        p_train_dict
    )  # {u: {i : 1 for i in the interacted items of user u}, ...}
...
def get_replay_learning_rank_dataset(
    T_rank_mat, rank_mat, sig_mat, args, num_replay_learning_sample
):

    # Sampling
    user_size, item_size = rank_mat.shape
    prob_mat_for_replay_learning = torch.exp(
        (T_rank_mat[:user_size, :item_size] - rank_mat) * args.eps
    )

    prob_mat_for_replay_learning = prob_mat_for_replay_learning * (
        rank_mat < args.absolute
    )
    items_for_replay_learning = torch.multinomial(
        prob_mat_for_replay_learning, num_replay_learning_sample
    )
    ...
    for u, items in enumerate(items_for_replay_learning):
        ...
        Dataset_for_replay_learning += list(
            zip([u] * len(rating), items.tolist(), rating.tolist())
        )

    return Dataset_for_replay_learning


def get_total_replay_learning_dataset_Teacher(
    T_score_mat,
    S_score_mat,
    S_rank_mat,
    P_score_mat,
    ...
    if S_score_mat is not None:
        S_replay_learning_dataset = get_replay_learning_rank_dataset(
            T_rank_mat, S_score_mat, S_rank_mat, args, args.S_sample
        )

    if P_score_mat is not None:
        P_replay_learning_dataset = get_replay_learning_rank_dataset(
            T_rank_mat, P_score_mat, P_rank_mat, args, args.P_sample
        )

    if CL_score_mat is not None:
        CL_replay_learning_dataset = get_replay_learning_rank_dataset(
            T_rank_mat, CL_score_mat, CL_rank_mat, args, args.CL_sample
        )
    ...
    replay_learning_dataset = (
        S_replay_learning_dataset
        + P_replay_learning_dataset
        + CL_replay_learning_dataset
    )
    print(
        f"\tTotal_replay_learning_dataset Before Filtering = {len(replay_learning_dataset)}"
    )
    ...
    print(
        f"\tTotal_replay_learning_dataset After Filtering = {len(replay_learning_dataset)}"
    )

    return replay_learning_dataset


def get_total_replay_learning_dataset(
    W_score_mat, S_rank_mat, S_sig_mat, P_rank_mat, P_sig_mat, args
):
    ...
    if S_rank_mat is not None:
        S_replay_learning_dataset = get_replay_learning_rank_dataset(
            W_rank_mat, S_rank_mat, S_sig_mat, args, args.S_sample
        )

    if P_rank_mat is not None:
        P_replay_learning_dataset = get_replay_learning_rank_dataset(
            W_rank_mat, P_rank_mat, P_sig_mat, args, args.P_sample
        )
...
def make_before_rating_mat(
    ...
    mat = torch.sparse_coo_tensor(
        (user_idx, item_idx), value, (before_user_size, before_item_size)
    )
...
def get_common_ids(before_ids, present_ids):
    common_ids_mask = torch.eq(before_ids.unsqueeze(0), present_ids.unsqueeze(1)).any(
        dim=0
    )
...
def get_eval_Ensemble(
    ...
    print(f"\t[The Result of new users in {task_idx}-th Block]")
    new_user_results = get_eval_with_mat(
        new_user_train_mat, new_user_valid_mat, new_user_test_mat, E_sorted_mat, k_list
    )
    print(
        f"\tvalid_R20 = {new_user_results['valid']['R20']}, test_R20 = {new_user_results['test']['R20']}"
    )
...
def get_eval(model, gpu, train_loader, test_dataset, k_list):
    ...
    for test_user in test_mat:
        ...
        for mode in ["valid", "test"]:
            ...
            if mode == "valid":
                gt_mat = valid_mat
                already_seen_items = set(train_mat[test_user]) | set(
                    test_mat[test_user].keys()
                )

            elif mode == "test":
                gt_mat = test_mat
                already_seen_items = set(train_mat[test_user]) | set(
                    valid_mat[test_user].keys()
                )
            ...
            for k in k_list:
                ...
                # Hit & Recall
                eval_results[mode][f"P{k}"].append(
                    hit_k / min(k, len(gt_mat[test_user].keys()))
                )
                eval_results[mode][f"R{k}"].append(
                    hit_k / len(gt_mat[test_user].keys())
                )
...
def get_eval_with_mat(
    ...
    for test_user in test_users:
        ...
        for mode in ["valid", "test"]:
            ...
            try:
                if mode == "valid":
                    gt_mat = valid_mat
                    already_seen_items = set(train_mat[test_user].keys()) | set(
                        test_mat[test_user].keys()
                    )
    ...
    for mode in ["valid", "test"]:
        for k in k_list:
            eval_results[mode][f"P{k}"] = round(
                np.asarray(eval_results[mode][f"P{k}"]).mean(), 4
            )
            eval_results[mode][f"R{k}"] = round(
                np.asarray(eval_results[mode][f"R{k}"]).mean(), 4
            )
            eval_results[mode][f"N{k}"] = round(
                np.asarray(eval_results[mode][f"N{k}"]).mean(), 4
            )
    return eval_results


def train_epoch(
    train_loader, loss_type, model, model_type, optimizer, scaler, args, gpu, report
):  # , RRD_flag = False, URRD_lambda = 0.0, before_num_user = None):
    ...
    for mini_batch in train_loader:
        ...
        if model_type in ["BPR", "LightGCN"]:
            base_loss, UI_loss, IU_loss, UU_loss, II_loss, cluster_loss = model(
                mini_batch
            )
            batch_loss = (
                base_loss
...
def train_epoch_base_model(
    train_loader, loss_type, model, optimizer, scaler, gpu, report
):
...
def eval_epoch(
    ...
    with torch.no_grad():
        if CL_flag:
            results, score_mat, sorted_mat = get_eval(
                model.base_model, gpu, train_loader, test_dataset, k_list
            )
        else:
            results, score_mat, sorted_mat = get_eval(
                model, gpu, train_loader, test_dataset, k_list
            )

        report.update(
            {f"valid_{key}": value for key, value in results["valid"].items()}
        )
        ...
        if report["valid_R20"] > eval_args["best_score"]:
            ...
            eval_args["best_model"] = deepcopy(
                model.state_dict()
            )  # .state_dict()) # detach(), cpu() 필요할듯? 근데 detach()가 에러가나는데 파라미터가 없어서 그런건가?
            eval_args["score_mat"] = deepcopy(score_mat)  # .detach().cpu())
            eval_args["sorted_mat"] = deepcopy(sorted_mat)
            eval_args["patience"] = 0

            print(
                f"[Best Model is changed] best_valid_score = {eval_args['best_score']}, test_score = {eval_args['test_score']}"
            )
...
def RRD_epoch_only_once(
    ...
    for idx in range(iteration):
        ...
        with torch.cuda.amp.autocast():
            ...
            interesting_items = interesting_items.to(
                gpu
            )  # .type(torch.cuda.LongTensor)
            uninteresting_items = uninteresting_items.to(
                gpu
            )  # .type(torch.cuda.LongTensor)

            interesting_prediction = model.base_model.forward_multi_items(
                batch_user, interesting_items
            )
            uninteresting_prediction = model.base_model.forward_multi_items(
                batch_user, uninteresting_items
            )
            URRD_loss = relaxed_ranking_loss(
...
def RRD_epoch_frequency(RRD_loader, gpu, model, URRD_lambda, optimizer, scaler, report):
    ...
    for batch_user in RRD_loader:
        ...
        interesting_items, uninteresting_items = RRD_loader.dataset.get_samples(
            batch_user
        )
    ...
    # report
    epoch_loss["epoch_URRD_loss"] = round(
        epoch_loss["epoch_URRD_loss"] / len(RRD_loader), 4
    )
    report.update(epoch_loss)


def RRD_epoch_train_loader(
    train_loader, RRD_loader, gpu, model, URRD_lambda, optimizer, scaler, report
):
...
def IR_RRD(
    ...
    for idx in range(iteration):
        ...
        epoch_loss["epoch_IR_RRD_loss"] = round(
            epoch_loss["epoch_IR_RRD_loss"] / iteration, 4
        )
        report.update(epoch_loss)


def IR_RRD_item_frequency(
    IR_RRD_loader, gpu, model, IR_reg_lmbda, optimizer, scaler, report
):
    ...
    for batch_item in IR_RRD_loader:
        ...
        with torch.cuda.amp.autocast():
            ...
            interesting_users, uninteresting_users = IR_RRD_loader.dataset.get_samples(
                batch_item
            )  # .detach().cpu())
            ...
            interesting_user_prediction = model.base_model.forward_multi_users(
                interesting_users, batch_item
            )
            uninteresting_user_prediction = model.base_model.forward_multi_users(
                uninteresting_users, batch_item
            )
        ...
        epoch_loss["epoch_IR_RRD_loss"] = round(
            epoch_loss["epoch_IR_RRD_loss"] / len(IR_RRD_loader), 4
        )
        report.update(epoch_loss)
