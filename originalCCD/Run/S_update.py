...
def main(args):
    ...
    p_train_dict = total_train_dataset[
        f"TASK_{task_idx}"
    ]  # {u_1 : {i_1, i_2, i_3}, ..., }
    ...
    if distillation_idx == 0:
        Teacher_Ensemble_RatingMat_path = (
            f"../ckpt/{args.dataset}/Teacher/base_teacher/Ensemble/TASK_0_score_mat.pth"
        )
    ...
    T_score_mat = (
        torch.load(Teacher_Ensemble_RatingMat_path, map_location=gpu)["score_mat"]
        .detach()
        .cpu()
    ...
    D_Student = get_model(
        b_total_user, b_total_item, b_SNM, gpu, args, model_type, D_model_weight
    ).to(gpu)
    del b_R, b_SNM
    ...
    negatvie_exclude = torch.cat(
        [negatvie_exclude, torch.tensor(D_sorted_mat[:, :40])], dim=1
    )
    del D_sorted_mat
    ...
    if distillation_idx == 0:
        ...
        S_proxy = get_model(
            bb_total_user, bb_total_item, bb_SNM, gpu, args, model_type, S_proxy_weight
        ).to(gpu)
        P_proxy = get_model(
            bb_total_user, bb_total_item, bb_SNM, gpu, args, model_type, P_proxy_weight
        ).to(gpu)
        del bb_R, bb_SNM
    ...
    if args.replay_learning:
        ...
        replay_learning_dataset = get_total_replay_learning_dataset(
            D_score_mat, S_rank_mat, S_sig_mat, P_rank_mat, P_sig_mat, args
        )
    ...
    train_loader = DataLoader(
        BPR_train_dataset, batch_size=args.bs, shuffle=True, drop_last=False
    )
    ...
    for epoch in range(args.max_epoch):
        ...
        CF_time = time.time()
        print(
            f"epoch_CF_loss = {round(epoch_CF_loss / len(train_loader), 4)}, CF_time = {CF_time - start_time:.4f} seconds",
            end=" ",
        )
        ...
        if epoch % 5 == 0:
            ...
            if valid_score > eval_args["best_score"]:
                ...
                )  # deepcopy(model) # model.state_dict()
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--save_path", "--sp", type=str, default="../ckpt/Yelp/Student/LightGCN_1/Test"
    )
