...
def main(args):
    ...
    total_train_dataset, total_valid_dataset, total_test_dataset, total_item_list = load_data_as_dict(data_dict_path,
    ...
    p_train_interaction, p_train_mat, p_valid_mat, p_test_mat = get_train_valid_test_mat(distillation_idx,
                                                                                         total_train_dataset,
                                                                                         total_valid_dataset,
                                                                                         total_test_dataset)
    ...
    if distillation_idx >= 1:
        ...
        new_user_train_mat, new_user_valid_mat, new_user_test_mat = get_train_valid_test_mat_for_new_users(b_total_user,
                                                                                                           p_total_user,
                                                                                                           p_train_mat,
                                                                                                           p_valid_mat,
                                                                                                           p_test_mat)
        ...
        new_user_results = get_eval_with_mat(new_user_train_mat, new_user_valid_mat, new_user_test_mat, T_sorted_mat,
                                             args.k_list)
    ...
    RRD_train_dataset, IR_reg_train_dataset, BPR_train_dataset = get_RRD_IR_BPR_dataset(T_score_mat, negatvie_exclude,
                                                                                        p_train_dict, p_train_mat,
                                                                                        p_train_interaction,
                                                                                        p_total_user, p_total_item,
                                                                                        args.nns, args.nui, args.nuu)
    ...
    for epoch in range(args.max_epoch):
        ...
        if epoch % args.eval_cycle == 0:
            ...
            valid_list, test_list = get_CL_result(total_train_dataset, total_valid_dataset, total_test_dataset,
                                                  D_sorted_mat, args.k_list, current_task_idx=distillation_idx,
                                                  FB_flag=False, return_value=True)
            ...
            if distillation_idx >= 1:
                print(f"\n\t[The Result of new users in {distillation_idx}-th Block]")
                new_user_results = get_eval_with_mat(new_user_train_mat, new_user_valid_mat, new_user_test_mat,
                                                     D_sorted_mat, args.k_list)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--LWCKD_flag", "--lf", action=argparse.BooleanOptionalAction,
                        help="whether using LWC_KD or not (--lf or --no-lf)")
    parser.add_argument("--PIW_flag", "--pf", action=argparse.BooleanOptionalAction,
    ...
    parser.add_argument("--eval_average_acc", "--eaa", action=argparse.BooleanOptionalAction,
    ...
    parser.add_argument("--random_init", "--r", action=argparse.BooleanOptionalAction,
    ...
    # SAVE
    parser.add_argument("--one_iteration", "--oi", action=argparse.BooleanOptionalAction,
