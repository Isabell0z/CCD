...
def get_two_hop_emb(R, start_id, end_id, base_emb, dim, topk=5):
    R_s = R.to_sparse()
    two_hop_mat = torch.sparse.mm(R_s, R.T).to_dense()  # U x U
    two_hop_mat = two_hop_mat[:, :start_id]  # U x (Before_U)
    two_hop_mat = torch.where(two_hop_mat > 0, 1, 0)
    num_interaction = two_hop_mat.sum(1)

    Frequency = torch.sparse.sum(R_s, dim=1).to_dense()  # 1-D array
    Frequency = Frequency[:start_id]  # Before_U
    Frequency_argsort = torch.argsort(Frequency, descending=False)
    Frequency_Rank = torch.zeros_like(
        Frequency_argsort, device=Frequency_argsort.device
    )
    ...
    for u_id in range(start_id, end_id + 1):
        ...
        if num_u_interaction == 0:
            ...
            idxs2 = Frequency_argsort[~torch.isin(Frequency_argsort, idxs1)][
                -(topk - num_u_interaction) :
            ]
...
class PIW_LWCKD(nn.Module):
    def __init__(
        ...
        # Model2: PIW Module
        self.CSTM = nn.Parameter(
            torch.randn(num_cluster, dim, dim)
        )  # cluster-specific transformation matrix
        self.PSV = PIW_State_Vector(num_cluster, dim)  # MLP for PIW State Vector
        self.kl_loss = nn.KLDivLoss(reduction="batchmean")

        # weight init for CSTM & PSV
        nn.init.normal_(self.CSTM, mean=0.0, std=0.01)
        ...
        if self.LWCKD_flag:
            if self.model_type in ["TransformerSelf"]:
                ...
                UI_loss, IU_loss, UU_loss, II_loss, cluster_loss = self.LWCKD_PIW(
                    batch_user,
                    batch_pos_item,
                    self.before_user_emb,
                    user_emb,
                    self.before_item_emb,
                    item_emb,
                    self.cluster,
                )
                ...
                for i in range(self.num_layer):
                    layer_losses = self.LWCKD_PIW(
                        batch_user,
                        batch_pos_item,
                        self.before_user_emb[i],
                        user_embs[i],
                        self.before_item_emb[i],
                        item_embs[i],
                        self.cluster[i],
                    )
    ...
    def LWCKD_PIW(
        ...
        map_common_batch_user = torch.tensor(
            list(map(lambda x: self.before_user_mapping[x.item()], common_batch_user))
        )

        common_batch_item = get_common_ids(batch_pos_item, self.before_item_ids)
        map_common_batch_item = torch.tensor(
            list(map(lambda x: self.before_item_mapping[x.item()], common_batch_item))
        )

        batch_user_rating_mat = self.before_rating_mat.to_dense()[
            map_common_batch_user
        ].to(self.gpu)
        batch_item_rating_mat = (
            self.before_rating_mat.to_dense().t()[map_common_batch_item].to(self.gpu)
        )
        ...
        UI_loss = self.LWCKD(
            batch_before_user_emb, present_common_item_emb, batch_user_rating_mat
        )
        IU_loss = self.LWCKD(
        ...
        if self.PIW_flag:
            ...
            batch_PIW = (batch_PIW / (batch_PIW.sum() + 1e-8)) * batch_PIW.size(
                0
            )  # Scaling
    ...
    def update(
        ...
        if random_init:
            ...
            new_user_embedding = nn.Parameter(
                torch.randn(num_new_user, self.dim)
            )  # .to(self.gpu)
            new_item_embedding = nn.Parameter(
                torch.randn(num_new_item, self.dim)
            )  # .to(self.gpu)
            ...
            Norm_Mat_for_new_user = new_user_interactions / (
                new_user_interactions.sum(dim=1, keepdims=True) + 1e-8
            )
            one_hop_emb_for_user = Norm_Mat_for_new_user @ item_emb

            new_item_interactions = R.T[start_newitem_id:, :start_newuser_id]
            Norm_Mat_for_new_item = new_item_interactions / (
                new_item_interactions.sum(dim=1, keepdims=True) + 1e-8
            )
            ...
            if only_one_hop:
                ...
                # 2 hop (represenative)
                # R = R.to_sparse()
                two_hop_emb_for_user = get_two_hop_emb(
                    R, start_newuser_id, end_newuser_id, user_emb, self.dim, topk=topk
                )
                two_hop_emb_for_item = get_two_hop_emb(
                    R.T, start_newitem_id, end_newitem_id, item_emb, self.dim, topk=topk
                )

                # Result
                new_user_embedding = nn.Parameter(
                    (one_hop_emb_for_user + two_hop_emb_for_user) / 2
                )  # .to(self.gpu) # num_new_user x dim
                new_item_embedding = nn.Parameter(
                    (one_hop_emb_for_item + two_hop_emb_for_item) / 2
                )  # .to(self.gpu) # num_new_item x dim
        ...
        self.base_model.item_emb = nn.Embedding.from_pretrained(
            torch.cat([item_emb, new_item_embedding])
        ).to(self.gpu)
    ...
    def get_cluster_loss(self, item_emb, cluster):
        ...
        Q = torch.pow(
            1
            + (
                torch.sum((present_item_emb.unsqueeze(1) - cluster) ** 2, dim=-1)
                / self.v
            ),
            -((self.v + 1) / 2),
        )
...
class CL_VAE(nn.Module):
    ...
    def forward(self, mini_batch):
        ...
        # CF
        mini_batch = {
            key: values.to(self.gpu) for key, values in mini_batch.items()
        }  # key: "user" or "rating_vec"
        output = self.base_model.forward(mini_batch)
        base_loss = self.base_model.get_loss(output)

        # CL
        total_kl_loss = torch.tensor(0.0).to(self.gpu)

        if self.CL_flag:
            user_output, _, _, _ = output
            common_batch_user = get_common_ids(
                self.common_pos_user_ids, mini_batch["user"]
            )  # common_pos_user_ids에서 user는 (common_user + common_item에 의해 선택된 user)의 집합 -> PIW값이 존재하지 않을 수 있다.
            ...
            for u in common_batch_user:
                ...
                before_dist = self.before_score_mat[u.detach().cpu()][
                    i.detach().cpu()
                ].to(self.gpu)
                present_dist = user_output[indices].squeeze()[i]

                kl_loss = self.kl_loss(
                    present_dist.log() + 1e-8, before_dist.log() + 1e-8
                )
                if (
                    self.task_user_piw_mean
                    and u.item() in self.task_user_piw_mean.keys()
                ):
                    piw_value = torch.tensor(self.task_user_piw_mean[u.item()]).to(
                        self.gpu
                    )
                else:
    ...
    def update(
        ...
        if self.CL_flag:
            ...
            self.common_pos_user_ids = torch.tensor(
                list(self.common_interaction.keys())
            ).to(self.gpu)

            before_user_count, before_item_count = before_R.shape
            dataset = {
                "user": torch.arange(before_user_count).to(self.gpu),
        ...
        e_weight = nn.Parameter(
            torch.cat(
                [self.base_model.encoder.weight.data, e_extra_layer.weight.data], dim=1
            )
        )  # Dim x (items + @) (wx + b)
        e_bias = nn.Parameter(self.base_model.encoder.bias.data)
        d_weight = nn.Parameter(
            torch.cat(
                [self.base_model.decoder.weight.data, d_extra_layer.weight.data], dim=0
            )
        )  # (items + @) x Dim (wx + b)
        d_bias = nn.Parameter(
            torch.cat([self.base_model.decoder.bias.data, d_extra_layer.bias.data])
        )
        ...
        self.base_model.decoder = nn.Linear(self.dim, self.base_model.item_count).to(
            self.gpu
        )
    ...
    def forward(self, mini_batch):
        ...
        if self.CL_flag:
            ...
            for user_id in common_batch_user:
                items_id = torch.tensor(self.common_interaction[user_id.item()]).to(
                    self.gpu
                )
                user_id_in_batch = torch.where(mini_batch["user"] == user_id)[0].to(
                    self.gpu
                )
